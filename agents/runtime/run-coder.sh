...
write_audit() {
  ...
  jq -n \
    ...
    '{
      ...
      gemini:{outcome:"reviewer_only",reason:"Gemini is reserved for independent review/escalation and is not a coding fallback."}
    }' > "$AUDIT_PATH"
}
...
# Primary coding provider: use the OpenRouter free model while its daily resource is available.
if [[ -n "${OPENROUTER_API_KEY:-}" ]]; then
  echo "Trying primary OpenRouter/OpenCode coding provider: openrouter/$OPENROUTER_MODEL"
  set +e
  ...
  if [[ "$OPENROUTER_REASON" = "quota_or_rate_limit" ]] && has_workspace_changes; then
    ...
    echo "OpenRouter daily/quota resource ended after producing repository changes. Preserving the workspace and handing the candidate to DeepSeek."
  else
    reset_workspace
  fi
else
...
# Paid fallback: DeepSeek Flash. If OpenRouter exhausted its quota after making changes,
# DeepSeek continues from that same disposable workspace; otherwise it starts clean.
