...
PRESERVE_FOR_DEEPSEEK=0

# Primary coding provider: use the free OpenRouter model while its daily allowance is available.
if [[ -n "${OPENROUTER_API_KEY:-}" ]]; then
  echo "Trying primary OpenRouter coding provider: openrouter/$OPENROUTER_MODEL"
  set +e
  ...
  if [[ "$OPENROUTER_REASON" =~ ^(quota_or_rate_limit|timeout)$ ]] && has_workspace_changes; then
    OPENROUTER_OUTCOME="partial_candidate_handoff"
    PRESERVE_FOR_DEEPSEEK=1
    echo "OpenRouter stopped after producing repository changes. Preserving them for DeepSeek continuation."
  fi
else
  echo "OPENROUTER_API_KEY is not configured; skipping OpenRouter."
fi

if [[ "$PRESERVE_FOR_DEEPSEEK" -eq 0 ]]; then
  reset_workspace
fi

# Paid fallback: DeepSeek Flash. It either starts from a clean tree or continues a bounded OpenRouter partial candidate.
if [[ -n "${DEEPSEEK_API_KEY:-}" ]]; then
  echo "Trying DeepSeek fallback coding provider: deepseek/$DEEPSEEK_MODEL"
  set +e
  ...
  if [[ "$DEEPSEEK_REASON" =~ ^(quota_rate_or_balance_limit|timeout)$ ]] && has_workspace_changes; then
    DEEPSEEK_OUTCOME="partial_candidate_after_limit"
    write_audit "deepseek" "deepseek/$DEEPSEEK_MODEL" "partial_candidate_after_limit"
    echo "DeepSeek stopped after producing repository changes. Preserving the bounded candidate for deterministic validation."
    exit 0
  fi
else
